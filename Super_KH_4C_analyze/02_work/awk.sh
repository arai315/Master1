...
rm ./awk_$p/$p1\_total.fasta
rm ./awk_$p/$p1\_up.fasta
rm ./awk_$p/$p1\_down.fasta
...
seqkit grep -j 24 -nrp "$name" ./fasta.d/$n.fasta | seqkit subseq -j 24 -r ${start1}:${start2} >> ./awk_$p/$p1\_total.fasta
seqkit grep -j 24 -nrp "$name" ./fasta.d/$n.fasta | seqkit subseq -j 24 -r ${start1}:${start2} >> ./awk_$p/$p1\_up.fasta
fi

totalend=`echo $((end2-end1))`

if [ ${totalend} -gt 100 ]; then
seqkit grep -j 24 -nrp "$name" ./fasta.d/$n.fasta | seqkit subseq -j 24 -r ${end1}:${end2} >> ./awk_$p/$p1\_total.fasta
seqkit grep -j 24 -nrp "$name" ./fasta.d/$n.fasta | seqkit subseq -j 24 -r ${end1}:${end2} >> ./awk_$p/$p1\_down.fasta
fi
